  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6 stagger-item" style={{ animationDelay: '40ms' }}>
        <h1 className="text-2xl font-bold text-brand-forest">User Management</h1>
      </div>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-brand-forest/10 bg-brand-cream/30">
              ...
              <tbody>
                {(users || []).map((user, index: number) => (
                  <tr
                    key={user.id}
                    className="stagger-item border-b border-brand-forest/5 hover:bg-brand-cream/20"
                    style={{ animationDelay: `${140 + index * 45}ms` }}
                  >
                    <td className="px-4 py-3">
                      <p className="font-medium text-brand-forest">{user.full_name || 'Unnamed'}</p>
