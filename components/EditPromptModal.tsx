...
export function EditPromptModal({
  ...
  // Depends on `mounted` so it re-runs after the canvas is actually in the DOM.
  // (The first render with mounted=false returns null — canvas doesn't exist yet.)
  useEffect(() => {
    if (!mounted) return;
    const update = () => {
   ...
   *   The canvas fills its container via CSS, but the image inside uses object-contain,
   *   which can leave letterbox gaps (e.g. a 1:1 image in a tall container has blank
   *   space at top/bottom). Lasso points are recorded in CANVAS coordinates. We must
   *   subtract the letterbox offset and scale relative to the IMAGE area, not the full
   *   canvas — otherwise the mask is shifted/stretched vs the actual image pixels.
   *
   *   Output mask must match the reference image's pixel dimensions exactly.
   */
  function buildCompositeMask(): string | null {
    ...
    // Compute where object-contain places the actual image within the canvas.
    // This accounts for any letterboxing (blank gaps) the browser adds.
    const [arW, arH]   = imgAr.split(':').map(Number);
    const imageAspect  = arW / arH;
    const canvasAspect = c.width / c.height;

    let imgX: number, imgY: number, imgW: number, imgH: number;
    if (imageAspect > canvasAspect) {
      // Image wider than canvas → letterbox top and bottom
      imgW = c.width;
      imgH = c.width / imageAspect;
      imgX = 0;
      imgY = (c.height - imgH) / 2;
    } else {
      // Image taller than canvas → letterbox left and right
      imgH = c.height;
      imgW = c.height * imageAspect;
      imgX = (c.width - imgW) / 2;
      imgY = 0;
    }

    // Scale from image-area coordinates to output pixel coordinates
    ...
    for (const region of regions) {
      if (region.pts.length < 2) continue;
      ctx.beginPath();
      // Subtract letterbox offset before scaling so coordinates are image-relative
      ctx.moveTo((region.pts[0].x - imgX) * scaleX, (region.pts[0].y - imgY) * scaleY);
      for (let i = 1; i < region.pts.length; i++) {
        ctx.lineTo((region.pts[i].x - imgX) * scaleX, (region.pts[i].y - imgY) * scaleY);
      }
      ctx.closePath();
